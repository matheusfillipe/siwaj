...
- The panel carries a wifi mark (`View::serving`) while config mode is up, since e-paper holds its last image and would otherwise say nothing about the radio being on. The s3 repaints before the radio goes off: the weather it now has if the setup was completed, the plain offline face if it was abandoned. `Board::draw` therefore leaves the panel awake and `power_down` is what sleeps it, so one session can paint more than once. The emulator answers the panel and the bench controls from a second server on `PANEL_PORT` that outlives config mode, because a real panel keeps its image while the radio is off. Each `EspHttpServer` needs its own `ctrl_port` or the second refuses to start.
- Config mode serves only while it is being used: `Config::awake_window` (the saved `awakeMinutes`, 1..=60, defaulting to `CONFIG_MODE_IDLE` when nothing is stored) after the last request the server is dropped. The loop re-reads it every tick so a window saved mid-session takes effect at once and matches what `/api/status` reports, and the s3 deep sleeps into the next weather cycle. Requests count as activity from `serve_gz`/`serve_json`, so a new handler stays awake by default; `GET /api/status` opts out through `write_json` because a page watching the clock must not stop it reaching zero. Editing sends nothing on its own, so the page refetches its config while someone works.
