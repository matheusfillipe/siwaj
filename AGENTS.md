...
- The panel carries a wifi mark (`View::serving`) while config mode is up, since e-paper holds its last image and would otherwise say nothing about the radio being on. A configured s3 leaves setup by restarting, so the next boot's weather cycle repaints without the mark; an unconfigured one keeps the setup screen and sleeps. The emulator serves `/api/frame` from the same server config mode drops, so the mirror cannot read the panel while the device is asleep.
