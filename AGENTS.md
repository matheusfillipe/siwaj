...
- Config mode serves only while it is being used: `siwaj_core::CONFIG_MODE_IDLE` after the last request the server is dropped, and the s3 deep sleeps into the next weather cycle. `GET /api/status` reports the countdown and is the one endpoint that does not count as activity, so an open page watching the clock cannot stop it reaching zero.
